...
void GameController::initDrawersAndSounds()
{
	SpriteInfo drawers[] = {
		...
		{ IID_BOARD, 0, "whitedash.tga"},
	    { IID_WHITEPLAYER, 0 , "whiteplayer.tga"},
	    { IID_BLACKPLAYER, 0, "blackplayer.tga"},
		{ IID_STAR, 0,"whitedot.tga"}
	};

	SoundMapType::value_type sounds[] = {
		make_pair(SOUND_THEME, "MaterialThoughts.wav")
	};
...
void GameController::keyboardEvent(unsigned char key, int /* x */, int /* y */)
{
	...
	case 'x':			m_lastKeyHit = KEY_PRESS_DROP;  break;
...
void GameController::doSomething()
{
	...
	case cleanup:
		...
		break;
	case gameoverwinner:
		m_mainMessage = " Winner: " + m_gw->getWinnerName() + "!";
	...
	case init:
	{
		...
		//SoundFX().abortClip();     // TURNS OFF MUSIC
...
static void drawScore(string gameScoreText)
{
	static int RATE = 1;
	static GLfloat rgb[3] =
	{ static_cast<GLfloat>(.6), static_cast<GLfloat>(.6), static_cast<GLfloat>(.6) };
	...
	glColor3f(rgb[0], rgb[1], rgb[2]);
	outputStrokeCentered(SCORE_Y, SCORE_Z - 0.75, gameScoreText.c_str());
}